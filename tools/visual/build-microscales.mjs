...
// no dependency is shipped or installed in the website. Official alpha is kept.
...
try {
  ...
  const assets = await page.evaluate(async inputs => {
    ...
    for (const [name, img] of Object.entries(images)) {
      ...
      for (let i = 0; i < data.data.length; i += 4) {
        ...
        // Neutral artwork becomes ink; chromatic brand accents remain exact.
        if (Math.max(r, g, b) - Math.min(r, g, b) < 45) {
          data.data[i] = 32; data.data[i + 1] = 42; data.data[i + 2] = 39;
        }
      }
      ctx.putImageData(data, 0, 0);
      result[`aekr-${name}-ivory-v1.png`] = c.toDataURL('image/png');
    }
